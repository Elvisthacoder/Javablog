...
public class Calculator {
    ...
    public static Object calculate(ActiveRecord recordHome, String function, String field, String options) {
        Map<String, String> optionMap = Converters.convertSqlOptionStringToMap(options);
     ...
     * @return result of calculation
     */
    public static Object calculate(ActiveRecord recordHome, String function, String field, Map<String, String> options) {
        Object result= null;
        if (options == null) return result;
        
        Map<String, Object> inputs = new HashMap<String, Object>();
        inputs.putAll(options);
        
        if (options != null && options.containsKey(ActiveRecordConstants.key_custom_sql)) {
            String sql = options.get(ActiveRecordConstants.key_custom_sql);
            result = SqlServiceClient.retrieveObjectBySQL(sql, inputs);
        }
        else if (options != null && options.containsKey(ActiveRecordConstants.key_custom_sql_key)) {
            String sqlKey = options.get(ActiveRecordConstants.key_custom_sql_key);
            result = SqlServiceClient.retrieveObjectBySQLKey(sqlKey, inputs);
        }
    ...
    private static String constructSqlWithInclude(ActiveRecord recordHome, String function, String field, Map<String, String> options) {
    ...
    private static String constructSql(ActiveRecord recordHome, String function, String field, Map<String, String> options) {
        String theSql = constructSelectPart(recordHome, function, field, options);
        String conditionSql = null;
        if (options != null && options.containsKey(ActiveRecordConstants.key_conditions_sql)) {
            conditionSql = options.get(ActiveRecordConstants.key_conditions_sql);
     ...
     */
    private static String constructSelectPart(ActiveRecord recordHome, String function, String field, Map<String, String> options) {
     ...
     */
    public static String getCountSelectPart(ActiveRecord recordHome, Map<String, String> properties) {
        String table = recordHome.getTableName();
        String countSQL = "SELECT count(";

        boolean useUnique = false;
        if (properties != null && properties.size() > 0) {
            String unique = properties.get(ActiveRecordConstants.key_unique);
        ...
        if (useUnique) {
            ...
            if (pks == null || pks.length ==0) {
                fields = StringUtil.flattenArray(table, recordHome.getRowInfo().getColumnNames(), ",");
            }
            else {
                fields = StringUtil.flattenArray(table, pks, ",");
            }
    ...
    private static boolean hasKey(Map<String, String> properties, String key, String expectedValue) {
